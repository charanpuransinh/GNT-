...
import { LedgerPublicContract, NotWiredLedgerContract } from './ledgerPublicContract';
...
export class PaymentService {
  ...
  private ledgerContract: LedgerPublicContract;

  constructor(private prisma: PrismaClient, eventBus: EventBus, ledgerContract?: LedgerPublicContract) {
    ...
    // WIRING FIX (2026-08-28): ledger posting now goes through M10's PUBLIC contract,
    // not a direct write to M10's table. Injected at composition root once M10 is available.
    this.ledgerContract = ledgerContract || new NotWiredLedgerContract();
  }
  ...
  async processPayment(id: string, tenantId: string, userId: string, gatewayRef?: string, gatewayResponse?: Record<string, unknown>) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // NOTE: ledger posting moved OUT of this transaction — see below. Writing to
      // ledgerEntry (M10's table) can no longer happen inside M11's own prisma.$transaction,
      // since M11 must go through M10's PUBLIC service, not its own tx handle.
    });

    // WIRING FIX (2026-08-28): post journal entries via M10's PUBLIC contract instead of
    // writing straight to ledgerEntry. This is currently BLOCKED (not wired to real M10
    // service) — caught here so a payment can still complete even though bookkeeping
    // is pending, but it is logged loudly instead of silently skipped.
    try {
      await this.ledgerContract.postJournalEntries([
        {
          transactionId: id,
      ...
      console.error('[M11->M10 PENDING] Ledger posting not completed — M10 contract not wired yet:', err);
    }
