...
export const MODULE_MOUNTS: ReadonlyArray<ModuleMount> = [
  ...
  {
    load: async () => (await import('./modules/m01-foundation/routes/app.routes')).default,
    code: 'M01',
    path: '/api/v1/foundation',
    mounted: true,
  },
  {
    load: async () => (await import('./modules/m02-core-architecture/routes/auth.routes')).default,
    code: 'M02',
    path: '/api/v1/auth',
    mounted: true,
  },
  {
    load: async () => {
      const { default: deviceRoutes } = await import(
        './modules/m03-device-platform/routes/device.routes'
      );
      // टास्क #024 — E1: expired sessions की सफाई का job (unref — process नहीं रोकता)
      const { startSessionCleanupJob } = await import(
        './modules/m03-device-platform/services/session-cleanup'
      );
  ...
  {
    load: async () =>
      (await import('./modules/m04-company-management/routes/company.routes')).default,
    code: 'M04',
    path: '/api/v1/company',
    mounted: true,
  },
    ...
    load: async () => {
      ...
      const gstServiceForHandlers = {
        async calculateInputTax(data: {
          ...
          console.log(
            `[M07→M09] input tax credit noted for invoice ${data.invoice_id} (GSTR-2 table update pending)`
          );
        },
        async reverseInputTax(data: {
          ...
          console.log(
            `[M07→M09] input tax reversal noted for return ${data.return_id} (GSTR-2 table update pending)`
          );
        },
      };
      const ledgerServiceForHandlers = {
        async createPurchaseEntry(data: {
          ...
          const res = await invoiceLedgerSvc.postPurchaseInvoice(
            data.company_id,
            data.invoice_id,
            'system'
          );
        ...
        async createPurchaseReturnEntry(data: {
          return_id: string;
          company_id: string;
          supplier_id: string;
          amount: number;
          ...
          // असली debit-note voucher (Dr Sundry Creditors = Cr Purchases + Cr GST Input) —
          // purchase-invoice accrual का उल्टा। idempotent।
          const res = await invoiceLedgerSvc.postPurchaseReturn(
            data.company_id,
            data.return_id,
            'system'
          );
      ...
      const handlers = new PurchaseEventHandlers(
        stockServiceForHandlers,
        gstServiceForHandlers,
        ledgerServiceForHandlers,
        eventBus
      );
      ...
      return createPurchaseRouter(
        new PurchaseController(purchaseService),
        new PurchaseOrderController(poService)
      );
    },
  },
  {
    load: async () => (await import('./modules/m08-sales/routes/sales.routes')).default,
    code: 'M08',
    path: '/api/v1/sales',
    mounted: true,
  },
  ...
  {
    load: async () => (await import('./modules/m09-gst/routes/gst.routes')).default,
    code: 'M09',
    path: '/api/v1/gst',
    mounted: true,
  },
  ...
  {
    load: async () => (await import('./modules/m11-payment/routes')).default,
    code: 'M11',
    path: '/api/v1/payments',
    mounted: true,
  },
  {
    load: async () => (await import('./modules/m12-hr')).default,
    code: 'M12',
    path: '/api/v1/hr',
    mounted: true,
  },
  {
    load: async () => (await import('./modules/m13-automation')).initM13Module(),
    code: 'M13',
    path: '/api/v1/automation',
    mounted: true,
  },
  {
    load: async () => (await import('./modules/m14-import-export/routes')).default,
    code: 'M14',
    path: '/api/v1/imports',
    mounted: true,
  },
  {
    load: async () => (await import('./modules/m15-sync')).default,
    code: 'M15',
    path: '/api/v1/sync',
    mounted: true,
  },
  ...
  },
  {
    ...
    load: async () => {
      const [
        ...
        { PAYMENT_WEBHOOK_SUCCESS, PAYMENT_WEBHOOK_FAILED },
      ] = await Promise.all([
      ...
      bus.on(
        ...
        (event: { order_id: string; payload: Record<string, unknown> }) => {
      ...
      return createIntegrationRoutes(
        new IntegrationController(integrationService),
...
export const pendingMounts = (): ReadonlyArray<ModuleMount> =>
  MODULE_MOUNTS.filter((m) => !m.mounted);
