...
const exportSheet = {
  ...
  rows: [
    {
      InvoiceNo: 'EXP-M21-TEST-1',
      Buyer: 'Overseas Buyer LLC',
      HSN: '84713010',
      Currency: 'INR',
      Qty: '5',
      Product: 'Laptop',
...
describe.runIf(process.env.TEST_DB === '1')('M21 export adapter — live DB', () => {
  ...
  it('export sheet → M20 trade_job बनता है (party/product resolve होकर)', async () => {
    ...
    const job = await prisma.trade_job.findFirst({
      where: { company_id: EXPORT_COMPANY, reference_no: 'EXP-M21-TEST-1' },
    });
    ...
    const product = await prisma.product_master.findFirst({
      where: { company_id: EXPORT_COMPANY },
    });
