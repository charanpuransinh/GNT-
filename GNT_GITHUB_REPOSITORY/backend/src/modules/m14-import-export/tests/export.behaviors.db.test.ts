...
describe.runIf(process.env.TEST_DB === '1')('M14 export behaviours — live DB', () => {
  beforeAll(async () => {
    await prisma.company_master.upsert({
      where: { id: COMPANY_ID },
      update: {}, create: { id: COMPANY_ID, name: 'ExpBeh Co', code: 'EXPBEH-1' },
    });
    await cleanup();
    await prisma.party_master.create({
      data: { company_id: COMPANY_ID, party_type: 'customer', name: 'PDF Party', email: 'pdf@test.com' },
    });
  });
  afterAll(cleanup);
  ...
  it('PDF export writes a real PDF file (starts with %PDF-), not JSON', async () => {
    const job = await ExportService.createJob({
      tenantId: COMPANY_ID, name: 'p.pdf', format: 'pdf', sourceEntity: 'customer',
      columns: [{ field: 'name', header: 'Name' }, { field: 'email', header: 'Email' }] as unknown[],
      createdBy: USER_ID,
    });
  ...
  it('invoice entity exports real SalesInvoice rows', async () => {
    const inv = await prisma.salesInvoice.create({
      data: {
        companyId: COMPANY_ID, branchId: 'b1', customerId: 'c1',
        invoiceNumber: 'INV-EXPBEH-1', invoiceDate: new Date('2026-09-01'), dueDate: new Date('2026-09-30'),
        totalAmount: 100, totalTax: 18, totalDiscount: 0, netAmount: 100, roundOff: 0, grandTotal: 118,
      },
    });
    const job = await ExportService.createJob({
      tenantId: COMPANY_ID, name: 'i.csv', format: 'csv', sourceEntity: 'invoice', createdBy: USER_ID,
    });
    await ExportService.processJob(job.id, COMPANY_ID);
  ...
  it('unknown entity => job FAILED with a clear error, no fake COMPLETED', async () => {
    const job = await ExportService.createJob({
      tenantId: COMPANY_ID, name: 'x.csv', format: 'csv', sourceEntity: 'GLERP_THINGS', createdBy: USER_ID,
    });
    await expect(ExportService.processJob(job.id, COMPANY_ID)).rejects.toThrow(/Unsupported export entity/);
