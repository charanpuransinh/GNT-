...
import { OCRProposedFieldDTO, OCRProposedItemDTO, OCRResultDTO } from '../types/purchase.types';
...
export class OCRService {
  ...
  private buildOCRResult(parsed: ReturnType<typeof parseOCRText>): OCRResultDTO {
    ...
    const buildItem = (
      item: { product_name: string; quantity: number; rate: number; amount: number },
      confidence: number
    ): OCRProposedItemDTO => ({
  ...
  validateOCRReview(ocrResult: OCRResultDTO): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    const requiredFields: (keyof OCRResultDTO)[] = [
      'supplier_name',
      'invoice_number',
      'invoice_date',
      'total_amount',
