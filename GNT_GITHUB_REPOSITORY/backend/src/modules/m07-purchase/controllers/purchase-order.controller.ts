...
export class PurchaseOrderController {
  ...
  create = async (req: Request, res: Response) => {
    try {
      ...
      res
        ...
        .json({
          success: true,
          data: await this.service.createPO({
            ...data,
            company_id,
  ...
  list = async (req: Request, res: Response) => {
    try {
      const data = purchaseOrderQuerySchema.parse({
        ...req.query,
        company_id: requireTenant(req).companyId,
      });
      res.json({ success: true, data: await this.service.getPOs(data) });
    } catch (e: any) {
      res.status(400).json({ success: false, message: e.message });
    }
  };
  ...
  get = async (req: Request, res: Response) => {
    try {
      const company_id = String(requireTenant(req).companyId || '');
      if (!company_id) throw new Error('Company context is required');
      res.json({
        success: true,
        data: await this.service.getPOById(String(req.params.id), company_id),
      });
    } catch (e: any) {
  ...
  update = async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        data: await this.service.updatePO(String(req.params.id), company_id, data),
      });
    } catch (e: any) {
      res.status(400).json({ success: false, message: e.message });
    }
  ...
  send = async (req: Request, res: Response) => {
    try {
      const company_id = String(requireTenant(req).companyId || '');
      if (!company_id) throw new Error('Company context is required');
      res.json(await this.service.sendPO(String(req.params.id), company_id));
    } catch (e: any) {
      res.status(400).json({ success: false, message: e.message });
    }
  };
  ...
  receive = async (req: Request, res: Response) => {
    try {
      ...
      if (!company_id) throw new Error('Company context is required');
      res.json(
        await this.service.receivePO(
          String(req.params.id),
          company_id,
          req.body.received_quantities || {}
        )
      );
    } catch (e: any) {
      res.status(400).json({ success: false, message: e.message });
  ...
  cancel = async (req: Request, res: Response) => {
    try {
      const company_id = String(requireTenant(req).companyId || '');
      if (!company_id) throw new Error('Company context is required');
      res.json(await this.service.cancelPO(String(req.params.id), company_id));
    } catch (e: any) {
      res.status(400).json({ success: false, message: e.message });
    }
  };
  ...
  convert = async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        data: await this.service.convertPOToInvoice(String(req.params.id), company_id, userId),
      });
    } catch (e: any) {
      res.status(400).json({ success: false, message: e.message });
    }
