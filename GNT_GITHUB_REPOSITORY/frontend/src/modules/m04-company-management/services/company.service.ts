...
export const CompanyService = {
  async getProfile(): Promise<Company> { const r = await apiClient.get<ApiEnvelope<Company>>(`${BASE}/profile`); return r.data.data; },
  async updateProfile(data: Partial<Company>): Promise<Company> { const r = await apiClient.put<ApiEnvelope<Company>>(`${BASE}/profile`, data); return r.data.data; },
  async getBranches(): Promise<Branch[]> { const r = await apiClient.get<ApiEnvelope<Branch[]>>(`${BASE}/branches`); return r.data.data; },
  async createBranch(data: Partial<Branch>): Promise<Branch> { const r = await apiClient.post<ApiEnvelope<Branch>>(`${BASE}/branches`, data); return r.data.data; },
  async deleteBranch(id: string): Promise<void> { await apiClient.delete(`${BASE}/branches/${id}`); },
  async getFinancialYears(): Promise<FinancialYear[]> { const r = await apiClient.get<ApiEnvelope<FinancialYear[]>>(`${BASE}/financial-years`); return r.data.data; },
  async createFY(data: Partial<FinancialYear>): Promise<FinancialYear> { const r = await apiClient.post<ApiEnvelope<FinancialYear>>(`${BASE}/financial-years`, data); return r.data.data; },
