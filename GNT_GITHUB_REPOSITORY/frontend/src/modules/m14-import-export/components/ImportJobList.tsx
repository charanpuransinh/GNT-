...
export const ImportJobList: React.FC<Props> = ({ onSelectJob }) => {
  const { jobs, fetchJobs, cancelJob, retryJob, isLoading, error, clearError } = useImportStore();
          ...
          <tbody>
            {filtered.map((job) => (
              <tr key={job.id} className="border-t hover:bg-gray-50 cursor-pointer" onClick={() => onSelectJob?.(job)}>
                ...
                <td className="px-3 py-2">{job.processedRows}/{job.totalRows}</td>
                ...
                <td className="px-3 py-2">
                  {job.status === 'PENDING' || job.status === 'PROCESSING' ? (
                    ...
                    <button onClick={(e) => { e.stopPropagation(); retryJob(job.id); }} className="text-blue-600 hover:underline text-xs">Retry</button>
                  ) : null}
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="px-3 py-8 text-center text-gray-400">No import jobs found</td></tr>
            )}
          </tbody>
        </table>
      </div>
