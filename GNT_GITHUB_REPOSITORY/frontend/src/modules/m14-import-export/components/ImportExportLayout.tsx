/**
 * ╔═══════════════════════════════════════════════════════════════╗
 * ║  M14 IMPORT/EXPORT — LAYOUT COMPONENT                        ║
...
const navItems = [
  { key: 'imports' as const, label: 'Imports', icon: Upload, path: '/import-export/imports' },
  { key: 'exports' as const, label: 'Exports', icon: Download, path: '/import-export/exports' },
  { key: 'templates' as const, label: 'Templates', icon: LayoutTemplate, path: '/import-export/templates' },
];

const ImportExportLayout = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { sidebarOpen, setSidebarOpen, setActiveTab } = useImportExportStore();
      ...
      >
        ...
        <div className="flex items-center h-16 px-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            {sidebarOpen && (
              <div>
                <h1 className="text-sm font-bold text-slate-900 leading-tight">Import/Export</h1>
                <p className="text-[10px] text-slate-500 leading-tight">M14 Module</p>
              </div>
            )}
          </div>
        </div>
