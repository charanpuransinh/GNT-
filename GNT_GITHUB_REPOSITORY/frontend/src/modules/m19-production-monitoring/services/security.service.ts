...
export class SecurityService {
  ...
  async getAuditLogs(params: {
    ...
    const response = await apiClient.get<PaginatedResponse<AuditLogDTO>>(`${this.basePath}/audit/logs`, { params });
  ...
  async getLoginHistory(params: {
    ...
    const response = await apiClient.get<LoginHistoryDTO[]>(`${this.basePath}/audit/login-history`, { params });
    return response.data;
  }

  async getPermissionChanges(params: { companyId: string; userId?: string }): Promise<AuditLogDTO[]> {
    const response = await apiClient.get<AuditLogDTO[]>(`${this.basePath}/audit/permission-changes`, { params });
    return response.data;
  }
  ...
  async getSecurityEvents(params: {
    ...
    const response = await apiClient.get<SecurityEventDTO[]>(`${this.basePath}/security/events`, { params });
    return response.data;
  }
  ...
  async triggerAnomalyCheck(payload: {
    ...
    const response = await apiClient.post<{ anomalyDetected: boolean; events: SecurityEventDTO[] }>(`${this.basePath}/security/anomaly-check`, payload);
    return response.data;
  }
  ...
  async getSystemHealth(companyId: string): Promise<{ overall: string; services: SystemHealthDTO[] }> {
    const response = await apiClient.get<{ overall: string; services: SystemHealthDTO[] }>(`${this.basePath}/health/system`, { params: { companyId } });
    return response.data;
  }

  async getDatabaseHealth(): Promise<{ status: string; responseTimeMs: number; timestamp: string }> {
    const response = await apiClient.get<{ status: string; responseTimeMs: number; timestamp: string }>(`${this.basePath}/health/database`);
    return response.data;
  }

  async getServicesHealth(): Promise<SystemHealthDTO[]> {
    const response = await apiClient.get<SystemHealthDTO[]>(`${this.basePath}/health/services`);
    return response.data;
  }
}
