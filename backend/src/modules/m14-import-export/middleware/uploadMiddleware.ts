...
const storage = multer.diskStorage({
  destination: (req: Request, file: Express.Multer.File, cb) => {
    try {
      const tenantIdRaw = String(req.headers['x-tenant-id'] || 'default');
      // sanitize tenant id to allow only safe filename chars
      const tenantId = tenantIdRaw.replace(/[^a-zA-Z0-9-_]/g, '_');

      const baseDir = path.resolve(process.cwd(), 'uploads');
      const uploadDir = path.resolve(baseDir, tenantId, 'imports');

      // Prevent path traversal: uploadDir must be inside baseDir
  ...
  filename: (req: Request, file: Express.Multer.File, cb) => {
    ...
    // ensure original filename parts are not used directly
